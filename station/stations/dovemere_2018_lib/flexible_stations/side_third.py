...
from .common import make_demo_one_line, horizontal_layout
...
for p, pclass in enumerate(platform_classes):
    for s, sclass in enumerate(shelter_classes):
        demo_layout = make_demo_one_line(cb14[pclass][sclass], 4, 1)
...
for p, pclass in enumerate(platform_classes):
    for s, sclass in enumerate(shelter_classes):
        demo_layout = make_demo_one_line(cb14[pclass][sclass].T, 4, 1)
        if pclass == "concrete" and sclass == "shelter_2":
...
for p, pclass in enumerate(platform_classes):
    ...
    demo_layout = make_demo_one_line(cb14[pclass], 4, 1)
    if p > 0:
        demo_layout.notes.append("noshow")
    ...
    side_third_stations.append(
        AStation(
            ...
            callbacks={
                "select_tile_layout": 0,
                "select_sprite_layout": grf.DualCallback(default=cb14[pclass], purchase=layouts.index(demo_layout)),
                **common_cb,
            },
            make_foundation=True,
            extra_code=common_code,
...
for p, pclass in enumerate(platform_classes):
    ...
    demo_layout = make_demo_one_line(cb14[pclass].T, 4, 1)
    if p > 0:
        demo_layout.notes.append("noshow")
    ...
    side_third_stations.append(
        AStation(
            ...
            callbacks={
                "select_tile_layout": 0,
                "select_sprite_layout": grf.DualCallback(default=cb14[pclass].T, purchase=layouts.index(demo_layout)),
                **common_cb,
            },
            make_foundation=True,
            extra_code=common_code,
